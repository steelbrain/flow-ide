...
import type { StatusObject, Location, CoverageObject, TypeAtPosObject, OutlineOptions } from './types'
...
const skipOutlineText = ['const', 'function', 'type']

function processOutline (outlines: OutlineTree[], { showExport, showFunctionArgs }: OutlineOptions): OutlineTree[] {
  return outlines.map((outline: OutlineTree) => {
    let newTokenizedText
    if (outline.tokenizedText) {
      newTokenizedText = []
      let skipRest = false

      for (let i = 0; i < outline.tokenizedText.length; i++) {
        if (skipRest) {
          continue
        }

        const text = outline.tokenizedText[i]
        if (!showExport && text.kind === 'keyword' && text.value === 'export') {
          i++ // skip the 'export' and the following whitespace
          continue
        }

        if (text.kind === 'keyword' && skipOutlineText.includes(text.value)) {
          i++ // skip the unnecessary type information and the following whitespace
          continue
        }

        if (!showFunctionArgs && outline.kind === 'function' && text.kind === 'method') {
          skipRest = true // keep this, but skip everything from here
        }

        newTokenizedText.push(text)
      }
    }
    return { ...outline, tokenizedText: newTokenizedText }
  })
}

export function toOutline(result: string, options: OutlineOptions): Outline {
  const parsed = JSON.parse(result)
  const outlines: Outline = astToOutline(parsed)
  return { outlineTrees: processOutline(outlines.outlineTrees, options) }
}
